...
while option_selected != 8
  ...
  case option_selected
    ...
    when 5
      ...
      highest_price = ""
      product_hash.each do |id, data|
        if data[1] > highest_product_price.to_f
    ...
    when 6
      highest_product_price = 0
      product_hash.each do |id, data|
        if data[1] > highest_product_price
          highest_product_price = data[1]
        end
      end
      ...
      product_hash.each do |id, data|
        if data[1] < highest_product_price
          lowest_product_price = "Product code: #{id}, Item description: #{data[0]}, Price: #{data[1]}"
        end
      end

      puts "Lowest product price is:"
      puts "#{lowest_product_price}"
      puts
...
# TO MAKE PRODUCT CODE:
# product_code = rand(100..999)
# while products.has.key?(product_code)
#   product_code = rand(100..999)
# end
...
# WHEN USER QUITS (#8), I WILL CLOSE THE .TXT FILE, OPEN IT AGAIN, AND RE-WRITE THE HASH WITH CHANGES TO .TXT FILE.

# TO WRITE TO FILE IN PROPER FORMAT: DO THIS, BUT, WRITE TO FILE, RATHER THAN PUTS TO SCREEN.
#ALSO: MOVE CLOSE OF FILE TO *AFTER* THIS BLOCK OF CODE..
# product_hash.each do |id, data|
#    puts "#{id},#{data[0]},#{data[1]}"
# end

revised_product_list = File.open('new_products.txt', 'w')
product_hash.each do |id, data|
